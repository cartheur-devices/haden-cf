...
namespace Haden.NxtSharp
{
    ...
    public partial class NxtBrick : Component
    {
        ...
        /// </summary>
        public void Connect(BluetoothClient btClient)
        {
            openSerialPorts(btClient);
            initSettings();
            Comm.KeepAlive();
        ...
        public string COMPortName { get { return _comPortName; } set { _comPortName = value; } }
        ...
        private void openSerialPorts(BluetoothClient btClient)
        {
            Comm = new NxtCommunicator(COMPortName);

            BluetoothDeviceInfo[] bdi = btClient.DiscoverDevices();
            //Extract the NXT's name
            string deviceName = bdi[1].DeviceName;//Literally the setup with the Edimax dongle under dolores.
            ...
            do
            {
                try
                {
                    ...
                    btClient.Connect(new BluetoothEndPoint(bdi[1].DeviceAddress, ServiceName));
                }
            ...
            while (btClient == null & CurrentTries < NumRetries);
        ...
        public void StopPolling()
        {
            lock (this)
            {
                if (pollThread != null)
                {
                    ...
                    while (pollThread.IsBackground)
                    {//this was .IsAlive so watch it.
                        Thread.Sleep(100);
                    }
                    pollThread = null;
                }
