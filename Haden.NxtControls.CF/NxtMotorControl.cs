...
namespace Haden.NxtSharp {
	public partial class NxtMotorControl : UserControl {
		...
		private void updateView() {
            ...
            int _marginTop = 3;
            int _marginBottom = 3;
            int hMargin = 6;// Margin.Left + Margin.Right;
            int vMargin = 6;// Margin.Top + Margin.Bottom;

			if(Orientation == NxtMotorControlOrientation.Vertical) {
				// Set minimum size
                Size = new Size(36 + hMargin, 48 + vMargin + ButtonDistance);
                ...
                btnTurnClockwise.Left = _marginLeft;
                btnTurnClockwise.Top = _marginTop;
				...
				// Set minimum size
				Size = new Size(48 + hMargin + ButtonDistance, 36 + vMargin);
				...
				btnTurnCounterClockwise.Width = width;
				btnTurnCounterClockwise.Height = height;
                btnTurnCounterClockwise.Left = Width - (_marginRight + width);
                btnTurnCounterClockwise.Top = _marginTop;
		...
		private void TurnCW() {
			if(Motor == null) {
				MessageBox.Show("Can't turn - there is no motor connected to this control.", "Error", MessageBoxButtons.OK, MessageBoxIcon.Exclamation, MessageBoxDefaultButton.Button1);
			} else {
				if(!IsRunning) {
					Motor.Turn(-Power, 0);
					IsRunning = true;
